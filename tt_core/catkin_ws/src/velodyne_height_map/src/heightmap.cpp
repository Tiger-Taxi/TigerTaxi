...
#define MIN(x,y) ((x) < (y) ? (x) : (y)) //comment this line out for raw height
#define MAX(x,y) ((x) > (y) ? (x) : (y))
//#define LIDAR_HEIGHT 0.235
//#define LIDAR_HEIGHT //0.410 //0.432
//#define LIDAR_HEIGHT 0.622
//#define LIDAR_HEIGHT 0.965
//#define LIDAR_ANGLE 0.1396
...
void HeightMap::constructFullClouds(const VPointCloud::ConstPtr &scan,
                                    ...
                                    size_t &empty_count)
{
  float min[grid_dim_][grid_dim_]; //comment this line out for raw height
  ...
  for (unsigned i = 0; i < npoints; ++i) {
    ...
    if (x >= 0 && x < grid_dim_ && y >= 0 && y < grid_dim_ && init[x][y]) {
      //if ((max[x][y] + LIDAR_HEIGHT > height_diff_threshold_) ) {
      if ((max[x][y] - min[x][y] > height_diff_threshold_) ) {    
        ...
        obstacle_cloud_.points[obs_count].z = 0.0;
        ...
        clear_cloud_.points[empty_count].z = 0.0;
                                    ...
                                    size_t &empty_count)
{
  ...
  for (unsigned i = 0; i < npoints; ++i) {
    ...
    if (x >= 0 && x < grid_dim_ && y >= 0 && y < grid_dim_) {
      if (!init[x][y]) {
        min[x][y] = scan->points[i].z; //comment this line out for raw height
        max[x][y] = scan->points[i].z;
        num_obs[x][y] = 0;
        num_clear[x][y] = 0;
        init[x][y] = true;
      } else {
        min[x][y] = MIN(min[x][y], scan->points[i].z); //comment this line out for raw height
        max[x][y] = MAX(max[x][y], scan->points[i].z);
      }
    }
  }
